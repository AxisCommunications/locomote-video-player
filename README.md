...
> - `debugLogger` - Specifices if debug messages should be shown or not. `debugLogger` is an object which contains `true` or `false` values for `trace` and/or `js`. `true` values will lead to information being logged in either the Flash console or in the Javascript console. Example `debugLogger: { trace: true, js: false }` causes debug messages to be shown in Flash but not in the Javascript console. The default value is `false` for both.
