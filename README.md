...
> - `scaleUp` - Specifies if the video can be scaled up or not. The default value is `false`. Not applicable if player is configured with `exactFit`
> - `exactFit` - Specifies that the video be displayed at the height and width specified by the current height and width of the Stage. The default value is `false`.
